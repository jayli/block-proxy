...
- `ReverseConnectHandler` 到 `TunnelConnection` 的发送接口应为 async，或具备等价的入队完成语义；`CONNECT_OK` 必须完成入队后才启动 target -> tunnel relay，避免 relay DATA 排到 CONNECT_OK 前面。
...
8. `TunnelClient` 必须把 per-connection disconnect 事件通知给 `ReverseConnectHandler`，由 handler 按 `TunnelConnection` 身份筛选并关闭对应 reqid sessions；不能只在全量 `stop()` 时清理 target connections。
