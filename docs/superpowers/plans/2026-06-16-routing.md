...
**Architecture:** 纯 Python 解析 Xray protobuf 格式的 geodata 文件（无外部依赖），在 `proxy_core.py` 的连接路径中插入路由判断：直连规则优先 → 代理规则 → 默认动作。新增 tkinter 子进程窗口管理规则配置。
...
```python
class GeodataLoader:
    """Lazy-loading geodata file parser with caching."""

    def __init__(self, data_dir):
        self._data_dir = data_dir
        self._geosite_cache = None  # {tag: [(type, value), ...]}
        self._geoip_cache = None    # {code: [IPv4Network/IPv6Network, ...]}

    def _load_geosite(self):
        if self._geosite_cache is not None:
            return
        path = os.path.join(self._data_dir, "geosite.dat")
        if not os.path.exists(path):
            logger.warning("geosite.dat not found: %s", path)
            self._geosite_cache = {}
            return
        with open(path, "rb") as f:
            data = f.read()
        self._geosite_cache = parse_geosite_data(data)

    def _load_geoip(self):
        if self._geoip_cache is not None:
            return
        path = os.path.join(self._data_dir, "geoip.dat")
        if not os.path.exists(path):
            logger.warning("geoip.dat not found: %s", path)
            self._geoip_cache = {}
            return
        with open(path, "rb") as f:
            data = f.read()
        self._geoip_cache = parse_geoip_data(data)
...
```bash
git add client/geodata_loader.py client/tests/test_geodata_loader.py
git commit -m "feat(client): add GeodataLoader with lazy loading and caching"
```
...
class TestMatchGeosite:
    def _make_engine(self, geosite_data=None):
        ...
        engine._proxy_rules = []
        engine._loader = None
        engine._geosite_data = geosite_data or {}
        engine._geoip_data = {}
        return engine
        return engine
...
class RoutingEngine:
    ...
    def __init__(self, config, geodata_dir):
        ...
        self._geosite_data = None  # None = not loaded; {} = loaded but empty
        self._geoip_data = None    # None = not loaded; {} = loaded but empty

    def _ensure_loaded(self):
        if self._geosite_data is not None:
            return  # already loaded
        self._geosite_data = {}
        self._geoip_data = {}
        if not self._loader:
            return
        geosite_path = os.path.join(self._loader._data_dir, "geosite.dat")
        if os.path.exists(geosite_path):
            from geodata_loader import parse_geosite_data
            with open(geosite_path, "rb") as f:
                self._geosite_data = parse_geosite_data(f.read())
        geoip_path = os.path.join(self._loader._data_dir, "geoip.dat")
        if os.path.exists(geoip_path):
            from geodata_loader import parse_geoip_data
            with open(geoip_path, "rb") as f:
                self._geoip_data = parse_geoip_data(f.read())

    def _match_geosite(self, host, code):
        """Check if a domain matches geosite rules for the given country code."""
        self._ensure_loaded()
        if self._geosite_data is None:
            return False
        rules = self._geosite_data.get(code.lower(), [])
...
class TestMatchGeoip:
    def _make_engine(self, geoip_data=None):
        ...
        engine._proxy_rules = []
        engine._loader = None
        engine._geosite_data = {}
        engine._geoip_data = geoip_data or {}
        return engine
...
```python
    def _match_geoip(self, host, code):
        ...
        self._ensure_loaded()
        if self._geoip_data is None:
            return False
        networks = self._geoip_data.get(code.lower(), [])
        try:
...
```python
        ...
        self._routing = RoutingEngine(routing_config, geodata_dir)
```
...
def _parse_rules_text(text):
    """Parse a multi-line text into a list of non-empty, non-comment lines."""
    rules = []
