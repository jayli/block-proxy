...
**Architecture:** 纯 Python 解析 Xray protobuf 格式的 geodata 文件（无外部依赖，手写 wire format 解析器）。优先级链：私网 IP 直连（最高）→ routing.resolve()（直连规则 > 代理规则 > 默认动作）→ 上游代理。geodata 在代理启动线程中选择性 eager load（只加载规则用到的 geosite/geoip），加载失败或未知 tag 时取反规则不命中（安全降级）。UDP ASSOCIATE 不参与分流。`_select_route()` 先确定实际 route，供连接和 access log 共用，确保 direct/proxy 标记正确。
...
class TestGeodataLoader:
    ...
    def test_selective_loading_skips_geoip(self, tmp_path):
        ...
        domain = _build_domain(2, "test.com")
        site = _build_geosite("cn", [domain])
        (tmp_path / "geosite.dat").write_bytes(_build_geosite_list([site]))

        loader = GeodataLoader(str(tmp_path), load_geosite=True, load_geoip=False)
        assert loader.geosite_available is True
        assert loader.geoip_available is False
        assert loader.has_geosite("cn") is True
        assert loader.has_geosite("missing") is False
```
...
```bash
git add client/geodata_loader.py client/tests/test_geodata_loader.py
git commit -m "feat(client): add GeodataLoader with selective eager loading"
```
...
class TestResolve:
    ...
    def test_negated_rule_safe_when_tag_unknown(self):
        """geodata 可用但 tag 不存在时，取反规则也不应命中。"""
        engine = self._make_engine(
...
Run: `cd client && python -m pytest tests/test_routing.py::TestProxyCoreRouting tests/test_routing.py::TestConnectTargetRouting -v`
...
不需要在 `proxy_core.py` 重复定义路径 helper；统一复用 `routing._geodata_dir()`，避免打包路径逻辑分叉。

- [ ] **Step 4: 修改 _connect_target — 添加路由判断**

将 `_connect_target` 方法（约第 503-513 行）替换为 `_select_route` + `_connect_target`：
...
print('203.0.113.1 →', engine.resolve('203.0.113.1', is_domain=False))
"
```
...
- `baidu.com → direct`
- `google.com → proxy`
- `203.0.113.1 → proxy`（TEST-NET-3 保留地址，不应在 cn CIDR 中）
