...
- Codex 读取本文件 `AGENTS.md`；项目级技能位于 `.agents/skills/`（Codex 官方规范：仓库根目录扫描 `.agents/skills`）。
- Claude Code 读取 `CLAUDE.md`（内容与本文件保持一致）与 `.claude/skills/`（遗留副本）。
- 修改项目指引时请同步两份文件；若不再使用 Claude Code，可删除 `CLAUDE.md` 与 `.claude/skills/` 仅保留 Codex 格式。
