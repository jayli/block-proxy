...
async function createSession(port, username, password, clientId = 'integration-client') {
...
function openSse(port, sessionId, username, password, clientId = 'integration-client') {
  return new Promise((resolve, reject) => {
    const req = https.get({
      hostname: 'localhost',
      port,
      path: `/xhttp/stream?token=${tokenFor(username, password)}&sessionId=${sessionId}&clientId=${clientId}`,
      rejectUnauthorized: false,
...
async function authenticateClient(port, username, password) {
  ...
  const sessionId = await createSession(port, username, password, clientId);
  const sse = await openSse(port, sessionId, username, password, clientId);
