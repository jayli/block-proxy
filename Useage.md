...
- [快速开始](#快速开始)
- [服务端部署](#服务端部署)
  - [Docker 部署](#docker-部署推荐) （不再维护 Docker，优先命令行启动）
...
### 命令行安装&启动（OpenWRT / Linux / macOS）（推荐）
...
# 启动并设为开机自启
pm2 start block-proxy --interpreter bash -- -c /path/to/rule.js
pm2 save
pm2 startup
...
### Q: iOS 设置了 wifi http 代理但无法上网？

A: 如果 block-proxy 部署在路由器上且配置了用户名和密码认证，iOS 通过设置网络里的手动设置 http 代理时，iOS 会拒绝连接，原因是 Safari 安全限制，即带认证的代理地址不能与网关 IP 相同。解决方案：要么去掉代理认证，要么将 block-proxy 部署在路由器以外的独立设备上。通常在 openwrt 上重新绑定一个新 ip 就可以。

### Q: iOS 手机端用什么连接

用小火箭

### Q：socks over tls 有没有被 MITM 的风险
...
隧道需要长期在后台保持连接，Android 系统默认会为了省电杀掉后台应用，所以要在系统层面做两处配置（以 vivo 为例）：

- **允许高耗电**：设置 → 电池 → 后台耗电管理，将 BlockProxy 设为「允许后台高耗电」（或关闭该应用的电池优化）。否则锁屏、低电量时系统会杀掉应用进程，导致隧道断开。
- **加入自重启名单**：在 设置 或「i 管家」→ 应用管理 → 权限管理/自启动管理 中，允许 BlockProxy「自启动」和「后台运行」。这样应用被系统查杀或手机重启后能自动恢复并重新连接隧道。
