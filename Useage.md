...
BlockProxyClient 提供 android 客户端（适配于 Android 6+），Android 客户端只提供一种“双向隧道”模式连接 block-proxy 服务端。

## NAT 隧道反代

Block-Proxy 服务端和 BlockProxyClient 客户端通过私有协议简历双向隧道，目的是为了穿透 NAT 网络，实现内网穿透的代理。
...
安全起见，需要穿透隧道代理的资源必须匹配白名单，白名单在 block-proxy 配置面板中隧道配置Tab中添加。
