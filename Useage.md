...
- [服务端部署](#服务端部署)
  ...
  - [命令行安装 & 启动](#命令行安装--启动openwrt--linux--macos)
- [Web 管理界面](#web-管理界面)
- [端口说明](#端口说明)
- [HTTP 代理使用](#http-代理使用)
  - [设备端设置代理](#设备端设置代理)
...
2. **打开管理界面**：浏览器访问 `http://<服务器IP>:8003`，配置拦截规则
...
启动后在浏览器访问 `http://<服务器IP>:8003`。

> 如果 8003 端口无法访问，可先访问 `http://<服务器IP>:8001`（代理端口），页面会提示开启 Express 管理界面。
...
| 8003 | Web 管理界面 | 可选，浏览器打开配置拦截规则和查看状态 |
...
在浏览器访问管理界面 `http://<服务器IP>:8003`，使用页面底部的二维码扫码下载证书，或直接访问 `http://<服务器IP>:8003/fetchCrtFile`。
...
Block-Proxy 提供 macOS 客户端 **SocksClient**，用于在 Mac 上连接远程 block-proxy 服务端，支持套 TLS，支持分流规则。当然 SocksClient 可以链接任何 Socks 节点，只要是标准的 Socks5/http 协议即可。
...
根据 Mac 芯片选择对应版本：
- M系列芯片（arm架构） → `SocksClient-macos-arm64.zip`
- Intel（x86架构） → `SocksClient-macos-x86_64.zip`

下载后解压，将 `SocksClient.app` 拖入「应用程序」文件夹即可。

首次打开时，如果提示"无法验证开发者"：
1. 打开「系统设置」→「隐私与安全性」
2. 在页面底部找到 SocksClient 并点击「仍要打开」
3. 或终端运行：`xattr -d com.apple.quarantine /Applications/SocksClient.app`

### 客户端配置

点击菜单栏的 SocksClient 图标 → 节点配置窗口：
