...
class XhttpHandler {
  ...
  constructor(options) {
    ...
    this._livenessSweeper = setInterval(() => this._sweepStaleSessions(), this._livenessSweepMs);
    this._livenessSweeper.unref();
  }
  ...
  _handleUpload(req, res, subPath) {
    ...
    this._readBody(req, MAX_POST_BODY_SIZE, (err, body) => {
      ...
      if (!ok) {
        console.warn(`[xhttp] Upload queue overflow for session ${sessionId}, closing session`);
        this._closeSession(sessionId, 'upload-queue-overflow');
   ...
   */
  closeAll() {
    if (this._livenessSweeper) {
      clearInterval(this._livenessSweeper);
      this._livenessSweeper = null;
    }
    ...
    const loop = async () => {
      try {
        while (!session.uploadQueue.closed) {
          ...
          if (payload === null) {
            if (!session.closed && this._sessions.has(session.sessionId)) {
              console.warn(`[xhttp] Upload queue closed for session ${session.sessionId}, closing session`);
              this._closeSession(session.sessionId, 'upload-queue-closed');
            }
            break;
          }
   ...
   */
  _closeSession(sessionId, reason = 'closed') {
    const session = this._sessions.get(sessionId);
    if (!session) return;
  ...
  _sweepStaleSessions() {
    const now = Date.now();
    for (const [sid, session] of this._sessions) {
      if (session.sseRes && now - session.lastActivityAt > this._livenessTimeoutMs) {
        console.warn(`[xhttp] Session ${sid} idle ${now - session.lastActivityAt}ms, closing (liveness timeout)`);
