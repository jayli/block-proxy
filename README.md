...
> block-proxy 物理内存使用平均在 200M，如果在 1G 内存的机器上，可以配置好参数后，只启动 `npm run proxy`，运行内存则将至 100M。
...
- MITM：需要 MITM 的请求，既有请求 RT，也有 tls 解包，RT 会增加。因为 tls 加解密基于系统的 OpenSSL，速度取决于 CPU 算力，Node 单线程做调度完全够用了。
